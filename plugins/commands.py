...
async def send_cached_media_wrapper(
    client, chat_id, file_unique_id, file_id, 
    protect_content=False, caption=None, file_type=None):
        ...
        if convert_to:
            reply_markup = InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    ...
                    callback_data=f"cnv{convert_to.lower()[0]}{'p' if protect_content else 'n'}_{file_unique_id}"
                ),
                InlineKeyboardButton(
                    "Cancel",
                    callback_data=f"cnv{file_type.lower()[0]}{'p' if protect_content else 'n'}_{file_unique_id}"
                )]
            ])
...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            ...
            [
                InlineKeyboardButton('⚡ Subscribe ⚡', url=f"https://youtube.com/@gillharyana"),
            ],
            [
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                kk, file_unique_id = message.command[1].split("_", 1)
                pre = 'checksubp' if kk == 'filep' else 'checksub' 
                btn.append([InlineKeyboardButton(" 🔄 Try Again", callback_data=f"{pre}#{file_unique_id}")])
    ...
    if not files_:
        logger.error('Code reached at danger point.')
